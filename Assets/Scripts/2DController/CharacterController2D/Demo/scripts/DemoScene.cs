    ...
    void attack()
    {
        if (_controller.isGrounded)
        {
            if (pm.mode.Equals("speed") && Input.GetAxis("Horizontal") != 0)
            {
                ...
                comboCountdown += Time.deltaTime;
                ButtonDelay = 0.9f;
                atkController.attack("SpeedAttack");
